 ...
 * @since 0.1
 */
class RedeventModelRole extends JModel
{
  /**
   * item id
   *
	...
	function delete($cid = array())
	{
		...
		if (count( $cid ))
		{
			...
			$query = ' SELECT r.id ' 
			       ...
			       . ' WHERE r.role_id IN (' . implode(',', $cid).')';
			$this->_db->setQuery($query);
			$res = $this->_db->loadResult();
			
			if ($res) {
				$this->setError(0, 'COM_REDEVENT_ROLE_DELETE_ERROR_ROLES_ASSIGNED');
				return false;
			}
  ...
  function checkin()
  {
    if ($this->_id)
    {
      $group = & JTable::getInstance('Redevent_roles', '');
  ...
  function store($data)
  {
    $row =& $this->getTable('Redevent_roles','');
